...
// 1. Total de pedidos del cliente
$query_pedidos = "SELECT COUNT(*) as total FROM pedidos WHERE id_cliente = ?";
...
// 2. Pedidos en camino
$query_en_camino = "SELECT COUNT(*) as total FROM pedidos WHERE id_cliente = ? AND estado = 'en_camino'";
...
$query_entregados = "SELECT COUNT(*) as total FROM pedidos WHERE id_cliente = ? AND estado = 'entregado'";
...
$query_estados = "SELECT estado, COUNT(*) as total FROM pedidos WHERE id_cliente = ? GROUP BY estado";
$stmt = $conexion->prepare($query_estados);
$stmt->bind_param("i", $id_cliente);
$stmt->execute();
$resultado_estados = $stmt->get_result();
$pedidos_por_estado = [];
...
$query_datos_cliente = "SELECT c.*, u.nombre, u.apellidos, u.email, u.telefono, ci.nombre as ciudad 
                       FROM clientes c 
                       JOIN usuarios u ON c.id_usuario = u.id
                       JOIN ciudades ci ON c.id_ciudad = ci.id
                       WHERE c.id = ?";
$stmt = $conexion->prepare($query_datos_cliente);
$stmt->bind_param("i", $id_cliente);
$stmt->execute();
